...
public class RatingsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetBookRatings(int bookId)
    {
        ...
        var ratedItems = ratings.Select(r => new
        {
            UserId = r.UserId,
            BookId = r.BookId,
            Score = r.Score,
        ...
        var unratedReviews = reviews
            .Where(rev => !ratings.Any(r => r.UserId == rev.UserId))
            .Select(rev => new
            {
                UserId = rev.UserId,
                BookId = rev.BookId,
                ...
                Username = rev.User?.Username ?? "Anonim Kullanıcı"
            }).ToList();

        var result = ratedItems.Concat(unratedReviews).OrderByDescending(x => x.CreatedAt);
    ...
    public async Task<IActionResult> GetUserRatings(int userId)
    {
        ...
        var unratedReviews = reviews
            .Where(rev => !ratings.Any(r => r.BookId == rev.BookId))
            .Select(rev => new
            {
                UserId = rev.UserId,
                BookId = rev.BookId,
